...
def Utility(GT_path, evaluated_path):
    '''
    Function to evaulate the your results for SBMnet dataset, this code will generate a 'cm.txt' file in your result path to save all the metrics.
    ...
    for category in category_list:
        ...
        for video in os.listdir(GT_category_path):
            ...
            for file in GTs:
                if file.endswith('.jpg') or file.endswith('.png'):
                    ...
                    GT_img = imageio.imread(os.path.join(GT_video_path, file)) #background ground truth
                    ...
                    for file in files:              #read the first image in the video folder
                        if file.endswith('.jpg') or file.endswith('.png'):
                            ...
                            result_img = imageio.imread(os.path.join(evaluated_video_path, file))
                            break


                    AGE, pEPs, pCEPs, MSSSIM, PSNR, CQM = SBM_Evaluate.Evaluate(GT_img, result_img);
                    if MSSSIM > MSSSIM_max:
                        ...
                        MSSSIM_max = MSSSIM
            if GT_exist:
                print(f'found ground truth image for video {video}')
                ...
                print(f'no ground truth image found for video {video}')
        if video_num >0 :
        ...
        else:# no ground truth data in the category
            print(f'warning : no ground truth image found for category {category}')
            c_AGE = 99
            c_pEPs = 99
            c_pCEPs = 99
            c_MSSSIM = 0
...
if __name__ == '__main__':
    ...
    print(f'evaluation finished, results saved as cm.txt in {evaluated_path}')
